...
export default function HodRejectedRequests() {
  ...
  useEffect(() => {
    let alive = true;
    apiRequest("/api/hod/transcript-requests/rejected")
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Rejected Requests" description="Track requests that were returned to Clerk or rejected during HoD review." />
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <EmptyState
              icon={XCircle}
              title="No returned or rejected requests"
              description="Once HoD sends a request back to Clerk or rejects it, it will appear here."
              badge="History"
            />
          ) : (
            <Table>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell><StatusBadge status={row.status || "Returned"} /></TableCell>
